...
std::tuple<double, double, double> MIPProblemProbe::getVariablePropsConstraint(const int& constraint_idx)
{
    ...
    return std::make_tuple(static_cast<double>(bin_count) / static_cast<double>(var_count), static_cast<double>(int_count) / static_cast<double>(var_count), static_cast<double>(cont_count) / static_cast<double>(var_count));
}
...
void MIPProblemProbe::getInstanceConstraintRawValues(RawInstanceConstrVals& ricv)
{

    for (const auto& con : MP_ptr->constraints) {
        ricv.rhs_vals.push_back(con.getRHS());
        ricv.constr_sum_obj.push_back(MP_ptr->getConstraintSumObj(con.getConIdx(), false));
        ricv.constr_sum_abs_obj.push_back(MP_ptr->getConstraintSumObj(con.getConIdx(), true));
        ricv.constr_sum_rhslhs_vals.push_back(con.getSumRHSLHSRatio());
        ricv.constr_largest_rhslhs_vals.push_back(con.getLargestRHSLHSRatio());
        ricv.non_zeroes.push_back(con.getNumVar());
    }
}

int MIPProblemProbe::getInstanceEquality()
{
    int equality_constr_count = 0;
    for (const auto& con : MP_ptr->constraints) {
        if (con.getBoundType() == Equal) {
            ++equality_constr_count;
        }
    }
    return equality_constr_count;
}

void MIPProblemProbe::getInstanceObjTerms(vector<double>& obj_terms)
{

    for (const auto& obj_pair : MP_ptr->objective_fn) {
        obj_terms.push_back(obj_pair.second);
    }
}
