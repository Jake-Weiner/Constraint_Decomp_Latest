...
def calculateStats(data_list):
    ...
    # for no relaxed constraints, there is no data, so leave all values as 0
    if data_list:
    ...
    for decomp_type_idx, decomp_type in enumerate(Decomp_Types):
        for problem_idx, problem_type in enumerate(problem_types):
            ...
            for instance_idx, instance_name in enumerate(instance_names[problem_idx]):
                features_calculated_output_path = features_calculated_output_folder + "/" + decomp_type + "/" + problem_type + "/" + instance_name + "/Features_Calculated" + "/" + "Relaxed_Constraint_Statistics"
                #create output folders if necessary
                Path(features_calculated_output_path).mkdir(parents=True, exist_ok=True)
                normalised_data_input_folder =  processed_results_folders[decomp_type_idx] + "/" + problem_type + "/" + instance_name + "/Normalised_Data" + "/" + "Relaxed_Constraint_Statistics"
