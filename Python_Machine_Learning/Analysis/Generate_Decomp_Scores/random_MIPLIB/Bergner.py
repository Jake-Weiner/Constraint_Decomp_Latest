...
def main():
    ...
    for problem_type_idx, problem_type in enumerate(problem_types):
        ...
        for instance_idx, instance_name in enumerate(instance_names[problem_type_idx]):
            features_collated_path = "/home/jake/PhD/Machine_Learning/Processed_Results/Features_Calculated/" + problem_type + "/" + instance_name + "/Features_Collated/collated.csv"
            # relaxed_constraint_prop_path = features_calucated_folder + "/" + problem_type + "/" + instance_name + "/" + "Normalised_Data" + "/" + "Relaxed_Constraint_Statistics" + "/" + "single_stats.csv"
            df = pd.read_csv(features_collated_path)
            # reset the indexes
            df.reset_index(drop=True, inplace=True)
            RBA_scores = df[['Decomposition Index', "Relaxed Constraint Prop_Relaxed_Constraint_Statistics_single_stats"]].copy()
            RBA_scores.rename(columns={"Prop_Relaxed_Constraint_Statistics_single_stats": "RBA Scores"}, inplace=True)
