...
def main():
    ...
    NSGA_Generic_script_folder = "/home/jake/PhD/Decomposition/Massive/Machine_Learning/experiment_scripts/generic_scripts/Decomp_Creation_Scripts/NSGA"
    ...
    for problem_idx, problem_type in enumerate(problem_types):
        ...
        Path(script_root_folder + "/" + problem_type + "/" + "Decomps" + "/" + "NSGA").mkdir(parents=True, exist_ok=True)

        for instance_idx, instance_name in enumerate(instance_names[problem_idx]):

            #create bash scripts
            with open(script_root_folder + "/" + problem_type + "/" + "Decomps" + "/" + "NSGA" + "/batch" + str(instance_idx), "w") as NSGA_batch_output_fs:
                with open(NSGA_Generic_script_folder + "/" + "batch0", "r") as NSGA_batch_input_fs:
                    for line in NSGA_batch_input_fs:
                        if instance_placeholder in line:
                            NSGA_batch_output_fs.write(line.replace(instance_placeholder,instance_name))
                        elif problem_type_placeholder in line:
                            NSGA_batch_output_fs.write(line.replace(problem_type_placeholder, problem_type))
                        else:
                            NSGA_batch_output_fs.write(line)

            #create job scripts
            with open(script_root_folder + "/" + problem_type + "/" + "Decomps" + "/" + "NSGA" + "/job" + str(instance_idx) + "_script", "w") as NSGA_job_output_fs:
                with open(NSGA_Generic_script_folder + "/" + "job0.script", "r") as NSGA_job_input_fs:
                    for line in NSGA_job_input_fs:
                        if job_batchnumber_placeholder in line:
                            NSGA_job_output_fs.write(line.replace(job_batchnumber_placeholder,str(instance_idx)))
                        else:
                            NSGA_job_output_fs.write(line)

        #create the run_all script to run each batch job
        with open(script_root_folder + "/" + problem_type + "/" + "Decomps" + "/" + "NSGA" + "/" + "run_all", "w") as run_all_output_fs:
