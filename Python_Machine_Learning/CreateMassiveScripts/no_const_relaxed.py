...
def main():
    ...
    Generic_script_folder = "/home/jake/PhD/Decomposition/Massive/Machine_Learning/experiment_scripts/generic_scripts/Gather_Statistics_Scripts/No_Constr_Relaxed"
    ...
    for problem_idx, problem_type in enumerate(problem_types):
        ...
        for instance_idx, instance_name in enumerate(instance_names[problem_idx]):
            # create bash scripts
            output_folder = output_root_folder + "/" + problem_type + "/" + action_name + "/No_Con_Relaxed"
            ...
            job_requirements = JobRequirements(job_batchnumber_placeholder, instance_idx,
                                               ...
                                               jobname_placeholder,
                                               instance_name + "_No_Const_Relaxed_Gather_Statistics")
        ...
        # then copy it in
        if not Path(
                output_root_folder + "/" + problem_type + "/" + action_name + "/No_Con_Relaxed" + "/" + "run_every_test").is_file():
            shutil.copyfile(Generic_script_folder + "/" + "run_every_test",
                            output_root_folder + "/" + problem_type + "/" + action_name + "/No_Con_Relaxed" + "/" + "run_every_test")

        # if chmod all files hasn't been copied across
        if not Path(output_root_folder + "/" + problem_type + "/" + action_name + "/No_Con_Relaxed" + "/" + "change_file_permissions").is_file():
            shutil.copyfile(Generic_script_folder + "/" + "change_file_permissions",
                            output_root_folder + "/" + problem_type + "/" + action_name + "/No_Con_Relaxed" + "/" + "change_file_permissions")
