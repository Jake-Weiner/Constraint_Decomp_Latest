...
class JobRequirements:
    def __init__(self, job_batchnumber_placeholder, job_batchnumber, run_time_placeholder, runtime,
                 memory_placeholder, memory, job_name_placeholder, job_name):
        self.job_batchnumber_placeholder = job_batchnumber_placeholder
        self.job_batchnumber = job_batchnumber
        ...
        with open(input_filepath, "r") as input_fs:
            for line in input_fs:
                ...
                if job_requirements.job_batchnumber_placeholder in line:
                    new_line = new_line.replace(job_requirements.job_batchnumber_placeholder, str(job_requirements.job_batchnumber))
...
def main():
    ...
    Generic_script_folder = "/home/jake/PhD/Decomposition/Massive/Machine_Learning/experiment_scripts/generic_scripts/Gather_Statistics_Scripts/Instance_Statistics"
    ...
    problem_types_runtimes = ["01:00:00", "01:00:00", "01:00:00"]
    problem_types_memory = ["8192", "8192", "8192"]
    ...
    for problem_idx, problem_type in enumerate(problem_types):
        ...
        for instance_idx, instance_name in enumerate(instance_names[problem_idx]):
            # create bash scripts
            output_folder = output_root_folder + "/" + problem_type + "/" + action_name + "/Instance_Statistics"
        ...
        # then copy it in
        if not Path(
                output_root_folder + "/" + problem_type + "/" + action_name + "/Instance_Statistics" + "/" + "run_every_test").is_file():
            shutil.copyfile(Generic_script_folder + "/" + "run_every_test",
                            output_root_folder + "/" + problem_type + "/" + action_name + "/Instance_Statistics" + "/" + "run_every_test")

        # if chmod all files hasn't been copied across
        if not Path(output_root_folder + "/" + problem_type + "/" + action_name + "/Instance_Statistics" + "/" + "change_file_permissions").is_file():
            shutil.copyfile(Generic_script_folder + "/" + "change_file_permissions",
                            output_root_folder + "/" + problem_type + "/" + action_name + "/Instance_Statistics" + "/" + "change_file_permissions")
